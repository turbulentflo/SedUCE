...
* An example input csv file with ficticious SL observations (*example_input_sl.csv*)
...
run_seduce("example_input_sl.csv", "example_output_predictions.csv")
```

Alternatively, you can run the model from a command prompt:
```bash
python seduce example_input_sl.csv example_output_predictions.csv
```

## Input variables

The provided implementation requires 9 input variables to produce uncertainty estimates on sediment load (SL) observations. A description of each is provided below. For a more detailed breakdown, please refer to Tables 2 and 3 of the accompanying [manuscript]().
...
LC_method | Load calculation method used for deriving SL <br>from Q and SSC measurements | 1 = mean sediment discharge of samples<br>2 = mean SSC of samples x mean Q of samples<br>3 = mean SSC of samples x mean daily Q<br>4 = Q-weighted mean SSC of samples x mean daily Q<br>5 = log-linear rating curve<br>6 = log-linear rating curve with Ferguson correction factor<br>7 = rating curve with non-linear regression
MP_length | Length of the measuring period <br>Units: yrs | [1, ∞[ <br>*Model was trained with values 1, 3, 5, 10, 15, and 20\**
IA_method | Interannual aggregation method used to derive long-term SL | 0 = not applicable (if MP_length = 1) <br>1 = average of annual SLs <br>2 = pooling samples from all years
Confidence_interval | Probability that the true SL falls within the <br>generated range of SL or error values <br>Units: %  |[1, 100]
...
SL_obs <br>*(optional)* |  Observed sediment load value. If using the <br>provided Python script to run SedUCE, this <br>field will be used to convert the predicted <br>error to a corresponding SL value. <br>Units: t yr<sup>-1</sup> | [0, ∞[
...
SL_error_pred_low | Estimated SL error corresponding to the lower percentile bound
SL_error_pred_high | Estimated SL error corresponding to the upper percentile bound
SL_pred_low | If SL_obs is given. Lower bound of the expected SL range. <br> Units: t yr<sup>-1</sup>
SL_pred_high | If SL_obs is given. Upper bound of the expected SL range. <br> Units: t yr<sup>-1</sup>
...
For questions or feedback regarding the SL uncertainty model, please contact turbulentflo(at)protonmail.com
